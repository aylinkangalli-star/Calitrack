...
def _status(next_cal: str | None) -> tuple[str, str, str]:
...
def show(user: dict):
    ...
    # ── Summary metrics ───────────────────────────────────────────────────────
    total = len(devices)
    overdue = sum(1 for d in devices if _status(d["next_calibration_date"])[0].startswith("Overdue"))
    due_soon = sum(1 for d in devices if _status(d["next_calibration_date"])[0].startswith("Due"))
    ...
    for d in devices:
        state, label, icon = _status(d["next_calibration_date"])
